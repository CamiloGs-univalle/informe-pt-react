/**
 * views/common/DriveExplorer.jsx
 * ─────────────────────────────────────────────────────────────────────────
 * Google Drive folder/file browser used inside the "Nuevo informe" wizard
 * (automatic mode) to let the user pick Excel files without downloading
 * them manually first. All Drive API calls go through
...
import {
  ...
  getStoredToken, setAccessToken, clearAccessToken,
  getDriveUserInfo, listAllContents, downloadFile,
...
export default function DriveExplorer({ onFilesSelected }) {
  ...
  const initDrive = useCallback(async (token) => {
    try {
      setLoading(true);
